...
const assetPool = [
  ...
  { src: '/chicken.jpeg', alt: 'chicken' },
]

// ─── UPDATED VIDEO DATA WITH ACTUAL THUMBNAILS ───
const videoPool = [
  { 
    src: '/pawpaw.mp4', 
    poster: '/pawpaw-thumb.jpeg', // Assumption: asset exists in /public
    title: 'Pawpaw Plantation Management' 
  },
  { 
    src: '/village.mp4', 
    poster: '/village-thumb.jpeg', // Assumption: asset exists in /public
    title: 'Teso Village Outreach Program' 
  },
]
...
export const About: React.FC = () => {
  ...
  const liveImages: GalleryItem[] = layoutSlots.map((slot, index) => {
    const calculatedIndex = activeIndices[index] ?? index
    const asset = assetPool[calculatedIndex % assetPool.length]
    return {
      src: asset.src,
      alt: asset.alt,
